...
export default function Dashboard() {
  ...
  const { data: budgetSummary } = useQuery({
    ...
    queryFn: () => api.budgets.summary(currentMonth()),
  });
      ...
      <div className="px-4 py-5 space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-3 animate-fade-up">
            ...
            />
            <StatsCard
              label={`Not Visited ${monthName}`}
              value={stats.notVisitedThisMonth ?? 0}
              color="#a78bfa"
              icon={<IconCalendarX />}
            />
            <Link href="/budget">
              <StatsCard
                label="Deals Budget"
                value={budgetSummary ? `$${budgetSummary.remaining.toFixed(0)}` : '—'}
                color="#f59e0b"
                sub={budgetSummary ? `$${budgetSummary.spent.toFixed(0)} / $${budgetSummary.budget.toFixed(0)}` : undefined}
                icon={<IconDollar />}
              />
            </Link>
          </div>
